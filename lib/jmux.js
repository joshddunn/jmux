...
export function configFile(file) {
  const filePath = file || `${os.homedir()}/.jmux.yml`
  return YAML.parse(readFileSync(filePath, 'utf8'))
}

export function start(name, config, attach = true) {
  const result = execSync(tmuxSessions()).toString()

  if (!result.includes(`${name}:`)) {
    execSync(newSession(name, config))
  }

  if (attach) {
    kexec(tmuxAttachSession(name))
  }
}
...
export function startAll(config) {
  ...
  kexec(tmuxAttachSession(envs[0]))
}

export function stop(name) {
  execSync(tmuxKillSession(name))
}
...
export function newSession(name, config) {
    ...
    windowConfig.panes.forEach((paneConfig, i) => {
      ...
      switch (i) {
        case 0:
          ...
          break;
        case 1:
          const splitPercent = undefinedFallback(windowConfig.splitPercent, 35)
  ...
  command.push(tmuxSelectWindow(0))
